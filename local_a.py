...
def gradient_batch(model, xpys, f, start_time):
    global grad_q
    start = time.time()
    ...
    pickle.dump((time.time() - start_time, model), f)     
...
def main(model, log=False):
    ...
    while time.time() - start_time < total_time:
        ...
        for i in minibatches:
            model, s = gradient_batch(model, i, f, start_time)
            saves += s
            if time.time() - start_time > total_time:
                break;
